...
pygame.display.set_caption("Ala-Too vs AUCA. Tic-tac-toe battle.")
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            if player_win or ai_win:
                game_over = True
                if player_win:
                    pygame.display.set_caption("Ala-Too won!")
                else:
                    pygame.display.set_caption("AUCA won!")
            elif field[0].count("x") + field[0].count("0") + field[1].count("x") + \
                    field[1].count("0") + field[2].count("x") + field[2].count("0") == 8:
                game_over = True
                pygame.display.set_caption("Nobody won:D")
